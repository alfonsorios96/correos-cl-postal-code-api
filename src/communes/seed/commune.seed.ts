...
async function seedCommunes() {
  ...
  for (const [regionNumberStr, communes] of Object.entries(communesByRegion)) {
    ...
    if (!region) {
      console.warn(
        `⛔ Region ${regionNumber} not found. Skipping ${communes.length} communes.`,
      );
      continue;
    }
    ...
    if (communesToInsert.length > 0) {
      ...
      console.log(
        `✅ Inserted ${communesToInsert.length} communes into ${region.label}.`,
      );
    } else {
      console.log(`⏩ All communes for ${region.label} already exist.`);
    }
  }

  await AppDataSource.destroy();
  console.log('🌱 Commune seeding completed.');
}

seedCommunes().catch((error) => {
  console.error('❌ Error during commune seeding:', error);
  process.exit(1);
});
